...
# Carregando o arquivo CSV do GitHub (comentado para uso local)
#df = pd.read_csv('https://raw.githubusercontent.com/vqrca/dashboard_salarios_dados/refs/heads/main/dados-imersao-final.csv')
# Carregando o arquivo CSV localmente
...
st.markdown('⬅️ Utilize os filtros à esquerda para refinar sua pesquisa.')
st.markdown('---')
...
with col_graf1:
    if not df_filtrado.empty:
        ...
        grafico_cargos = px.bar(top_cargos, x='usd', y='cargo', orientation='h', title='Top 10 Cargos com Maior Salário Médio', labels={'usd': 'Salário Médio (USD)', 'cargo': 'Cargo'})
        grafico_cargos.update_layout(title_x=0.1, yaxis={'categoryorder': 'total ascending'})
        st.plotly_chart(grafico_cargos, use_container_width=True)
    else:
        st.warning('Nenhuma informação disponível para os filtros selecionados.')
...
with col_graf4:
    if not df_filtrado.empty:
        ...
        grafico_paises = px.choropleth(media_ds_pais, locations='residencia_iso3', color='usd', color_continuous_scale='rdylgn', title='Salário médio anual de Cientista de Dados por país', labels={'usd': 'Salário médio (USD)', 'residencia_iso3': 'País'})
